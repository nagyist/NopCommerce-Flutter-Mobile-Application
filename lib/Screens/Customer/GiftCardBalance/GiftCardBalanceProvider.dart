...
import 'package:nopcommerce/Screens/Customer/GiftCardBalance/Models/GetGiftCardBalanceResponseModel.dart';
import 'package:nopcommerce/Services/CommonService.dart';
import 'package:nopcommerce/Services/GiftCardBalanceService.dart';
import 'package:nopcommerce/Models/CustomProperties.dart';
import 'package:nopcommerce/Widgets/DialogBoxWidget.dart';

class GiftCardBalanceProvider extends ChangeNotifier {

  bool isPageLoader = true;
  bool isAPILoader = false;
  TextEditingController giftCodeController = new TextEditingController();
  late GetGiftCardBalanceResponseModel getGiftCardBalanceResponseModel;
  ...
  pageLoadData({required BuildContext context}){
    ...
    isPageLoader = true;
    isAPILoader = false;
    giftCodeController.text="";
    notifyListeners();
    getGiftCard(context: context);
  }

  getGiftCard({required BuildContext context}) async{
    isAPILoader = true;
    notifyListeners();
    Response response = await GiftCardBalanceService().getGiftCardBalance(context: context);
    if(response.statusCode==200){
      getGiftCardBalanceResponseModel=getGiftCardBalanceResponseModelFromJson(response.body);
  ...
  buttonClickEvent({required BuildContext context})async
  {
    isAPILoader=true;
    notifyListeners();
    Response response= await GiftCardBalanceService().applyGiftCardBalance(context: context,param:"/${giftCodeController.text.trim().toString()}");
    if(response.statusCode==200){
      getGiftCardBalanceResponseModel=getGiftCardBalanceResponseModelFromJson(response.body);
      isAPILoader=false;
    }else if(response.statusCode==400){
